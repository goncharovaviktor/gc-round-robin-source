# GC Manager Round Robin for Cloudflare v2.2.0
...
- D1, обе Queue, Durable Object, bindings и схема D1 создаются автоматически
  во время первого Deploy; `database_id` вводить не требуется.
...
- Каждый GitHub Deploy сначала выполняет 24 unit- и 13 интеграционных тестов;
...
нужен: инфраструктура Cloudflare создаётся самой сборкой после запуска Deploy
из публичного GitHub-репозитория. Не переключайте рабочий процесс GetCourse на
новый URL, пока `/health` не покажет `ok: true` и не пройдены проверки тестовых
пользователей.
